 ...
 */
 ...
 /**
 * Circle Auth `Strategy` constructor.
 *
 * Options:
 ...
 * Examples:
 *
 *     passport.use(new CircleAuthStrategy(
 *       {
 ...
 */
 function Strategy(options, verify) {
   ...
   if (!verify) { throw new TypeError('CircleAuthStrategy requires a verify callback') }
   ...
   circleauthwrapper.configure(options)
 ...
 */
 Strategy.prototype.authenticate = async function(req, options) {
   ...
   if (!userID || !sessionID || !authID || !authType || !signature || (!isTwoFactorValid && !isLoginValid)) {
     return self.redirect(`https://circleaccess.circlesecurity.ai/login/${self._appKey}`)
   }
   ...
   function verifiedCallback(err, user, info) {
     ...
     if (!user) {
       if (authType === 'login') {
         return self.redirect(`https://console.circlesecurity.ai/dashboard/login_email/index?appKey=${self._appKey}`)
       } else {
   ...
   try {
     const userSession = await circleauthwrapper.getUserSession(sessionID, userID)
     if (!userSession || !userSession.data || (userSession.data.status !== 'active' && userSession.data.status !== 'usedOnce')) {
       throw Error('Invalid Circle Auth session.')
     }
