 ...
 */
 Strategy.prototype.authenticate = async function(req, options) {
   ...
   if (!userID || !sessionID || !authID || !authType || !signature || (!isTwoFactorValid && !isLoginValid)) {
     return self.redirect(`https://circleauth.gocircle.ai/login/${self._appKey}`)
   }
   ...
   function verifiedCallback(err, user, info) {
     ...
     if (!user) {
       if (authType === 'login') {
         return self.redirect(`https://console.gocircle.ai/dashboard/login_email/index?appKey=${self._appKey}`)
       } else {
